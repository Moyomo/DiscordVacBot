...
*/
export async function entryDetails(interaction) {

    let steam_id = interaction.customId.slice(8);
    ...
    try {
        ...
        if (json.response.players.length == 0) {
            ...
            await interaction.editReply({ content: `${interaction.user}`, embeds: [errorEmbed], ephemeral: true });
            return;
        }
    ...
    await interaction.editReply({ content: `${interaction.user}`, embeds: [suspectInfoEmbed, watchlistInfoEmbed] });
}
